...
function computeOffset(opcode, address, version1, version2) {
 if(opcode == 0x50 || opcode == 0xD4 || opcode == 0xD5 || opcode == 0xD6 || opcode == 0xC1) { return 0; }
 let offset = 0;
 for(offset = 0; offsetTable[offset] != null; ++offset) { if(address - offsetTable[offset][version1 + 1] < offsetTable[offset][0]) { break; } }
 --offset;
...
function translateCode(code, version1, version2) {
 ...
 if(matches === null) { return "Invalid code " + code; }
 if(matches.length !== 1) { return "Invalid code " + code; }
 if(matches[0] !== code) { return "Invalid code " + code; }
...
function translateCodes() {
 let select1 = document.getElementById("version1");
 let select2 = document.getElementById("version2");
 let version1 = select1.options[select1.selectedIndex].value;
 let version2 = select2.options[select2.selectedIndex].value;
 let version1Codes = document.getElementById("version1Codes").value.split("\n").filter(code => { return code.search(/[^\s]/g) !== -1; }).map(code => { return code.trim().replace(/\s+/g, " "); });
 let version2Codes = [];
 for(let i = 0; i < version1Codes.length; i++) { version2Codes.push(translateCode(version1Codes[i], versionMap[version1], versionMap[version2])); }
